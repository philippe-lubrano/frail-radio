...
/**
 * Scan public/jumpUp and public/liquid for .mp3 files and return a playlist
 * array sorted alphabetically within each genre.
 *
 * Each entry: { title: string, file: string, genre: 'jumpUp' | 'liquid' }
 *
 * `file` is the public URL path (e.g. "/jumpUp/Overdrive.mp3").
 */
export function getPlaylist() {
  const publicDir = path.join(process.cwd(), 'public')
  const genres = ['jumpUp', 'liquid']
